...
- `services/policyd` now also exports its own runtime service-payload manifest, so the service graph can name a concrete `policyd-payload` artifact without changing the live `init.capsule` boot lane yet.
