...
Each service crate exports its own descriptor and launch spec. `apps/initd` assembles those service-owned specs into the first runtime manifest and publishes a deterministic kernel-binding plan for required services. The runtime-spine services now also carry standalone payload manifests and validated build artifacts (`policyd-payload`, `agentd-payload`, `memd-payload`, `acceld-payload`, `intentd-payload`, `scened-payload`, `appbridged-payload`, and `shelld-payload`), but those artifacts remain outside the live boot lane until real service launch integration exists. Optional `shelld` remains model-only until an operator shell boundary is requested.
...
The shared IDs, descriptors, service statuses, launch specs, kernel bindings, launch requests, launch results, policy decisions, intent envelopes, typed intent projections, scene-switch requests, app-action requests, scene IDs, app IDs, and agent IDs live in `libs/nova_rt::service`.

`services/shelld` now parses operator commands for the first runtime spine, projects launch/status/scene-switch commands into the shared `intentd` path, and exposes typed service-status, scene-list, memory-placement-plan, accelerator-dispatch-plan, and intent-projection output lines over generic `nova_rt` descriptors for inspecting runtime state. `services/scened` now owns typed scene manifests, checkpoints, restore plans, root scene bindings, and a first scene-switch planner over shared `nova_rt` requests. `services/appbridged` now owns manifest-backed app action support and routes supported, unsupported, and approval-needed app actions, including requests projected through shared `nova_rt` app-action types. `services/intentd` has locked route coverage for status, scene switching, app opening, launch requests, explicit target overrides, scene-scoped policy request projection into `policyd`, and typed downstream projections into shared launch/status/scene-switch/app-action requests. `services/agentd` now owns agent lifecycle labels, runtime records, quota snapshots/decisions for tool grants, service delegation, and memory pages, plus scene participation checks. `services/memd` now owns typed memory placement requests and plans over UMA, discrete, NVLink, and MIG profile pools. `services/acceld` now owns typed dispatch requests and plans over CPU, GB10, RTX, and Hopper placeholder backends plus fabric queue classes. This gives the operator shell, scene manager, app bridge, policy service, intent router, agent manager, memory manager, and accelerator manager a shared service vocabulary without introducing kernel dependencies.
