...
- the sidecar also has an empty-by-default bootstrap frame-arena descriptor in the ABI, and `novaaa64` now populates it with loader-reserved `LOADER_DATA` pages when the bootstrap user-window policy is valid. This lets the kernel prove backing-frame and page-table descriptor readiness and construct a transitional AArch64 translation-table set in reserved frames while still deferring TTBR/TCR/MAIR installation and real MMU enable.
