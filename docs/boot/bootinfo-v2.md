...
- carry an optional bootstrap frame-arena descriptor so the EL0 planner can use loader-reserved physical backing pages without treating v1 capsule fields as free memory
...
- the sidecar also has an empty-by-default bootstrap frame-arena descriptor in the ABI, and `novaaa64` now populates it with loader-reserved `LOADER_DATA` pages when the bootstrap user-window policy is valid. This lets the kernel prove backing-frame and page-table descriptor readiness, populate payload/context/stack backing frames, construct a transitional AArch64 translation-table set, and prepare TTBR/TCR/MAIR values while still deferring register installation and real MMU enable.
