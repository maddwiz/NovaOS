...
- The feature-gated EL0 diagnostic now disables the inherited translation regime for that probe only, enters the bootstrap payload with `eret` to EL0, routes `initd`'s first `svc` through the lower-EL handler, and returns to EL0 spin. This proves the raw privilege-transition mechanics but is not the final user-mapped task model.
