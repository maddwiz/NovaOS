...
- The loader now publishes a portable BootInfo v2 bootstrap user-window policy when the embedded bootstrap payload descriptor is valid, so the kernel EL0 planner reports `bootstrap el0 mapping ready` instead of rejecting the old firmware-table placeholder.
- The loader also reserves a transitional BootInfo v2 bootstrap frame arena from `LOADER_DATA` pages, so the kernel can now report `bootstrap el0 backing frames ready`, `bootstrap el0 page tables ready`, `bootstrap el0 backing frames populated ready`, `bootstrap el0 page tables constructed ready`, and `bootstrap el0 mmu registers prepared ready`. The kernel copies the bootstrap payload/context into planned backing frames, zeroes their padding plus the planned user stack, writes a fixed six-page AArch64 translation-table set into the remaining arena space for the planned kernel/user descriptors, and computes the TTBR/TCR/MAIR values. It still does not install those registers or enable the MMU.
- The next step toward the full kernel path is still explicit and narrow: install the prepared TTBR/TCR/MAIR state under controlled exception coverage, replace the no-MMU EL0 diagnostic shortcut with real user mappings/stacks, and turn the current in-place bootstrap payload transfer plus typed bootstrap context into a real isolated `initd` task boundary.
