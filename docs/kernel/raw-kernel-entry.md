...
- The loader also reserves a transitional BootInfo v2 bootstrap frame arena from `LOADER_DATA` pages, so the kernel can now report `bootstrap el0 backing frames ready` and `bootstrap el0 page tables ready` for descriptor planning. This is still planning evidence only; it does not install live page tables or enable the MMU.
- The next step toward the full kernel path is still explicit and narrow: turn the ready page-table descriptors into real page-table construction, then replace the no-MMU EL0 diagnostic shortcut with real user mappings/stacks and turn the current in-place bootstrap payload transfer plus typed bootstrap context into a real isolated `initd` task boundary.
