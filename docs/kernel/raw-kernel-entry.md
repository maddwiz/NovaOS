...
- The loader also reserves a transitional BootInfo v2 bootstrap frame arena from `LOADER_DATA` pages, so the kernel can now report `bootstrap el0 backing frames ready`, `bootstrap el0 page tables ready`, `bootstrap el0 backing frames populated ready`, and `bootstrap el0 page tables constructed ready`. The kernel copies the bootstrap payload/context into planned backing frames, zeroes their padding plus the planned user stack, and writes a fixed six-page AArch64 translation-table set into the remaining arena space for the planned kernel/user descriptors, but it does not install those tables into TTBR/TCR/MAIR or enable the MMU.
- The next step toward the full kernel path is still explicit and narrow: install the constructed page tables, replace the no-MMU EL0 diagnostic shortcut with real user mappings/stacks, and turn the current in-place bootstrap payload transfer plus typed bootstrap context into a real isolated `initd` task boundary.
