...
- The next step toward the full kernel path is still explicit and narrow: replace the no-MMU EL0 diagnostic shortcut with real user mappings/stacks, then turn the current in-place bootstrap payload transfer plus typed bootstrap context into a real isolated `initd` task boundary.
