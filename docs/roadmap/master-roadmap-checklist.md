...
- `services/policyd`, `services/agentd`, `services/intentd`, `services/scened`, `services/appbridged`, and `services/shelld` now exist as additive runtime-spine service crates backed by shared `nova_rt` service, policy, agent, intent, scene, app bridge, service status, launch-spec, kernel-binding, and launch-result contracts, and `apps/initd` now publishes a typed local boot status page plus launch manifest and deterministic kernel-binding plan for the first required service chain while deferring optional `shelld`.
