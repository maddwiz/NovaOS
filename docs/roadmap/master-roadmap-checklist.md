...
- `services/acceld` and `services/memd` now expose backend/profile interfaces with placeholder CPU, GB10, RTX, Hopper, UMA, discrete, NVLink, and MIG lanes, and `memd` now has a static typed placement request/plan model for selecting supported profile pools without claiming real UMA residency policy yet.
