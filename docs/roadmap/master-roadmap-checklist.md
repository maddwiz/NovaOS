...
- `services/policyd`, `services/agentd`, `services/intentd`, `services/scened`, `services/appbridged`, and `services/shelld` now exist as additive runtime-spine service crates backed by shared `nova_rt` service, policy, agent, intent, scene, app bridge, service status, service-owned launch-spec, kernel-binding, and launch-result contracts, and `apps/initd` now publishes a typed local boot status page plus a launch manifest assembled from those service-owned specs, a deterministic kernel-binding plan for the first required service chain, and an operator-readable joined runtime report while deferring optional `shelld`.
