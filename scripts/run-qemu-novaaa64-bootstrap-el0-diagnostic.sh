...
set +e
EFI_BINARY=novaaa64 \
EXPECT_TEXTS="NovaOS stage0 loader;NovaOS stage1 entered;NovaOS stage1 bootinfo_v2 sidecar;NovaOS kernel entered;NovaOS kernel bring-up;init capsule summary observed;bootstrap task current initd;bootstrap task launch plan from bootinfo_v2;bootstrap el0 mapping ready;bootstrap el0 backing frames ready;bootstrap el0 page tables ready;bootstrap el0 backing frames populated ready;bootstrap el0 page tables constructed ready;bootstrap el0 mmu registers prepared ready;bootstrap task boundary same-el;bootstrap task target boundary drop-to-el0;bootstrap task transfer initd;bootstrap lower-el svc from initd;NovaOS bootstrap vector return" \
