...
LOG_FILE="${REPORT_DIR}/bootstrap-trap-diagnostic-${STAMP}.log"
LATEST_LOG_LINK="${REPORT_DIR}/latest-bootstrap-trap-diagnostic.log"
LATEST_LOG_PATH="${REPORT_DIR}/latest-bootstrap-trap-diagnostic-path.txt"
LATEST_STATUS_FILE="${REPORT_DIR}/latest-bootstrap-trap-diagnostic-status.txt"
...
set +e
EFI_BINARY=novaaa64 \
EXPECT_TEXTS="NovaOS stage0 loader;NovaOS stage1 entered;NovaOS stage1 bootinfo_v2 sidecar;NovaOS kernel entered;NovaOS kernel bring-up;init capsule summary observed;bootstrap task current initd;bootstrap task launch plan from bootinfo_v2;bootstrap capability probe passed;bootstrap endpoint probe passed;bootstrap task transfer initd;NovaOS initd payload entered;NovaOS initd bootstrap context ready;bootstrap kernel call from initd;NovaOS initd bootstrap kernel call passed;NovaOS initd bootstrap trap begin" \
...
grep -q 'NovaOS initd bootstrap trap begin' "${LOG_FILE}" && trap_begin_seen=true
...
grep -q 'bootstrap live trap from initd' "${LOG_FILE}" && trap_handler_seen=true
grep -q 'NovaOS initd bootstrap trap passed' "${LOG_FILE}" && trap_return_seen=true

overall_status="trap_not_reached"
exit_code=1
if ${trap_return_seen}; then
  overall_status="returned"
  exit_code=0
elif ${trap_handler_seen}; then
  ...
  exit_code=0
elif ${trap_begin_seen}; then
  overall_status="reached_trap_no_vector"
  exit_code=0
fi
...
novaos_write_latest_status "${LATEST_STATUS_FILE}" \
  ...
  "trap_begin_seen=${trap_begin_seen}" \
  ...
  "trap_handler_seen=${trap_handler_seen}" \
  "trap_return_seen=${trap_return_seen}" \
