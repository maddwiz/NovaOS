...
EFI_BINARY=novaaa64 \
EXPECT_TEXTS="NovaOS stage0 loader;NovaOS stage1 entered;NovaOS stage1 bootinfo_v2 sidecar;NovaOS kernel entered;NovaOS kernel bring-up;init capsule summary observed;bootstrap task current initd;bootstrap task launch plan from bootinfo_v2;bootstrap capability probe passed;bootstrap endpoint probe passed;bootstrap shared memory probe passed;bootstrap lower-el svc dry-run passed;bootstrap task transfer initd;bootstrap task boundary same-el;NovaOS initd payload entered;NovaOS initd bootstrap context ready;bootstrap kernel call from initd;NovaOS initd bootstrap kernel call passed;NovaOS initd bootstrap svc passed" \
