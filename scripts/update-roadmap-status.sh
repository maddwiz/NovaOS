...
- the typed \`init.capsule\` path now enforces a narrow task-bound bootstrap policy, carries a validated bootstrap payload descriptor and empty-by-default bootstrap user-window descriptor through \`BootInfo v2\`, and now hands the payload a typed bootstrap context in \`x0\` that also supports a transitional same-EL bootstrap kernel-call round-trip, but the default transfer still runs in-place under the raw-kernel environment rather than as a real isolated \`initd\` task, and the current QEMU lane still only proves \`current_el=1\` with an explicitly logged unisolated \`current-el-svc\` current boundary plan plus a logged isolated \`drop-to-el0\`/\`el0-svc\` target boundary plan. The same-EL exception defect is now cleared in QEMU: the default smoke lane now proves payload-originated \`svc\` return, the lower-EL AArch64 vector path now has the correct ELR-advance handler proven by a kernel-owned dry-run, the feature-gated EL0 diagnostic now proves raw no-MMU \`eret\` to EL0 and lower-EL \`svc\` return to EL0 spin, and the kernel now has tested EL0 mapping-readiness, backing-frame arena, and page-table descriptor planners that rebase the embedded payload into a proposed user image window and reject the current placeholder bootinfo user-window contract. The kernel-owned \`svc\`, pre-transfer \`svc\`, payload-originated \`svc\`, and payload-originated \`brk\` diagnostics all return end-to-end, with the kernel-owned \`svc\` scalar caller capture matching \`x0/status\`, \`x1/value0\`, and \`x2/value1\`. Full capability objects, broader endpoint routing, shared-memory policy beyond the first reserved bootstrap region, populated user-window policy, real allocator-backed frame source, real page-table construction, real user mappings/stacks for the EL0 boundary, and a true isolated \`initd\` task still remain ahead
