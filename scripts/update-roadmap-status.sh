...
- the service graph is still a typed local runtime model; \`initd\` now publishes a typed local boot status page, launch manifest, deterministic kernel-binding plan, and joined operator report for the first service chain, but real kernel task creation, endpoint wiring, shared-memory grants, and kernel-backed service health publication are not integrated yet
